...
function plugSSM_check_installation_date() {
    ...
    $nobug = "";
    $nobug = get_option('plugSF_hide_notices');
...
function plugSSM_display_admin_notice() {
    ...
    $cfb_install_link =  esc_url( network_admin_url('plugin-install.php?tab=plugin-information&plugin=' . 'contact-form-add' . '&TB_iframe=true&width=100%&height=800' ) );
    ...
    echo '<div style="width:65%; display: inline-block;" ><br> <h3><b>' . __( 'Do You Like Subscribe Form ? You\'ll Love Form Builder! ', 'page-builder-add' ) . '</b></h3>';

    echo '<p style="font-size:16px;">' . __( 'Create & design high-end pixel perfect forms at record speed with our Drag & Drop form builder plugin. Install now its free. ', 'page-builder-add' ) . '</p>
...
function plugSSM_set_no_bug() {
    ...
    if ( 1 == $nobug ) {
        ...
        add_option( 'plugSF_hide_notices', TRUE );
